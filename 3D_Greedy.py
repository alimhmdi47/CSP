...
# Check if WIDTH, HEIGHT, DEPTH are set
...
# لیست محصولات با نام، عرض، ارتفاع و عمق
products = [
    {"name": "Item 1", "width": 3, "height": 2, "depth": 1},
    {"name": "Item 2", "width": 4, "height": 3, "depth": 2},
    {"name": "Item 3", "width": 5, "height": 2, "depth": 3},
    {"name": "Item 4", "width": 2, "height": 4, "depth": 1},
    {"name": "Item 5", "width": 3, "height": 3, "depth": 2},
    {"name": "Item 6", "width": 6, "height": 2, "depth": 4},
]

# لیست ابعاد و تعداد ترکیب‌های مختلف برای هر محصول
w, h, d = list(), list(), list()
m = list()

numberOfVariables = len(products)

for index, product in enumerate(products):
    dimensions = (product["width"], product["height"], product["depth"])
    possible_permutations = set(permutations(dimensions, 3))
...
for i in range(numberOfVariables):
    x.append(model.add_var(name=f'x_{i+1}', var_type=CONTINUOUS, lb=Real(0)))
    y.append(model.add_var(name=f'y_{i+1}', var_type=CONTINUOUS, lb=Real(0)))
    z.append(model.add_var(name=f'z_{i+1}', var_type=CONTINUOUS, lb=Real(0)))
...
for i in range(numberOfVariables):
    for j in range(i+1, numberOfVariables):
        model += left[i][j] + right[i][j] + under[i][j] + over[i][j] + behind[i][j] + infront[i][j] >= xsum(s[i][k] for k in range(m[i])) + xsum(s[j][k] for k in range(m[j])) - float(1)
...
# Solving
model.tolerances.mip_gap = 0.05
model.set_time_limit(600)
